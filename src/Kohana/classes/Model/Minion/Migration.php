<?php
namespace Ansilog\Model\Minion;

use Kohana, Kohana_Database, Kohana_Exception;
use DB, Database, View;
...
class Migration extends \Model {
	...
	{
		$query = $this->_db->query(Database::SELECT,
			"SELECT tablename FROM pg_catalog.pg_tables WHERE tablename = '".$this->_table."'");
	 ...
	 * @return string|Migration Get table name or return $this on set
	 */
	...
	{
		return DB::select('*', DB::expr('"group" || \':\' || CAST("timestamp" AS VARCHAR) AS "id"'))->from($this->_table);
	}
	...
	{
		DB::insert($this->_table, ['timestamp', 'group', 'description'])
	...
	public function get_migration($group, $timestamp = NULL)
	{
		if ($timestamp === NULL)
		{
			...
			{
				throw new Kohana_Exception('Invalid migration id :id', [':id' => $group]);
			}
	...
	public function fetch_current_versions($key = 'group', $value = NULL)
	{
		...
		$select = DB::select($key, 'timestamp', 'description', 'applied', 'id')
			->from(array(
				$this->_select()
				->where('applied', '=', TRUE)
			...
			->group_by('group', 'timestamp', 'description', 'applied', 'id');

		empty($value) OR $select->select($value);
	 ...
	 * required version
	 *
	 * @param  array  $group  The groups to get migrations for
	...
	public function fetch_required_migrations(array $group, $target = TRUE)
	{
		...
		foreach ($group as $group_name)
		{
			...
			{
				throw new Kohana_Exception("Migration group :group does not exist", [':group' => $group_name]);
			}
		}
